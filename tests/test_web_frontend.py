...
class WebFrontendTests(unittest.TestCase):
    ...
    def test_frontend_modules_expose_manifests(self):
        ...
        self.assertIn("diode-abm-dynamic", emitter)
        self.assertIn("IDSAT_F", emitter)
        self.assertIn("CISS_F", emitter)
        self.assertIn("PARAMS: TJ=25 LG=${params.lg}n RGEXT=0", emitter)
        self.assertIn('getPath(project, "parasitics.rg_ext_ohm")', emitter)
        html = (ROOT / "web" / "workbench_app.html").read_text(encoding="utf-8")
        self.assertIn("componentProfile", html)
        self.assertIn("Diode Compact Model", html)
        self.assertIn("Diode Behavioral Model", html)
