...
<img src="assets/diagrams/runtime-split.svg" alt="Runtime split from browser and local Python backend into the shared service contract">
...
tests, and future desktop adapters.

## Runtime Modes

The frontend should target one stable contract and then select an adapter:

- `browser-pages`: static browser files and local JavaScript only. This hardened
  mode avoids third-party CDN code and therefore supports JSON review and
  starter model export, while PDF extraction stays in the local Python backend.
...
- The static browser workbench should remain useful without installation, but
  the local backend is the authoritative path for high-fidelity extraction and
  verification.
